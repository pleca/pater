...
require_once(CMS_DIR . '/application/models/netto.php');

class NettoAdmin extends Netto
{
   private $db;
   private $tpl;
   private $table;
   ...
   public function load_admin($limitStart = 0, $limit = 100)
   {
   ...
   public function pages($limit = 50)
   {
   ...
   public function load_id($id = 0)
   {
      if($id > 0)
      {
         $q = "SELECT a.* FROM `".$this -> table."` a ";
         $q.= "WHERE a.id='".(int)$id."' ";
         if($v = $this -> db -> getRow($q))
         {
				$v = mstripslashes($v);
				return $v;
			}
   ...
   public function add($post)
   {
      ...
      $this -> tpl -> setInfo($GLOBALS['LANG']['info_add']);
      return $id;
   }
   ...
   public function edit($post)
   {
      ...
      $this -> tpl -> setInfo($GLOBALS['LANG']['info_edit']);
      return true;
   }
   ...
   function delete($id = 0)
   {
	...
	public function generate($post = '') {
		...
		require_once (CMS_DIR . '/application/libraries/tcpdf/tcpdf.php');
