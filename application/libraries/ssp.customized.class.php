        ...
        if (isset($request['columns'])) {
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' &&
					...
					SSP::decorateColumn($column);	//exclusion for sku
...
//dump($request['columns']);
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '')
    {
        ...
        if($joinQuery){
            ...
            $query =  "SELECT DISTINCT SQL_CALC_FOUND_ROWS ".implode(", ", $col)."
			 $joinQuery
			 $where
			 $extraWhere
			 $groupBy
...
//        dump($query);die;
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        try {
            ...
            SSP::fatal( "An SQL error occurred: ".$e->getMessage() . $sql);
//            SSP::fatal( "An SQL error occurred: ".$e->getMessage() . PHP_EOL . $sql);
        }
//dump($sql);
        // Return all
	...
	static function decorateColumn(&$column) {										
		if ($column['field'] == 'sku') {
			$column['db'] = '`v`.`sku`';
		}
