...
class TransportRegionPostcodeController {
	...
	public function addAction() {
		if (!$_POST) {
			...
			if(!$this->regionPostcode->getByRegionIdPost1($region['id'], $_POST['post1'])) {
				$_POST['region_id'] = $region['id'];
				if ($id = $this->regionPostcode->set($_POST)) {
	...
	public function deleteAction() {
		...
		if (!$aItem = $this->regionPostcode->getById($this->getParam('id'))[0]) {
			error_404();
		}
		
		if ($this->regionPostcode->deleteById($aItem['id'])) {
	...
	function addForm($params = []) {
		...
		$data = array(
			...
			'pageTitle' => $GLOBALS['LANG']['module_transport'] . ' | ' . $GLOBALS['LANG']['transport_courier'] . ' : ' . $courier['name']
				  ...
				  . ' | ' . $GLOBALS['LANG']['transport_postcode'] . ' | ' . $GLOBALS['LANG']['btn_add']
		);

		echo Cms::$twig->render('admin/transport_region_postcode/add.twig', array_merge($data, $params));		
	}
	...
	function listAction($params = []) {
		...
		if (!$entities = $this->regionPostcode->getAllByRegionId($region['id'])) {
			$entities = array();
		}
		...
		$data = array(
			...
			'pageTitle' => $GLOBALS['LANG']['module_transport'] . ' | ' . $GLOBALS['LANG']['transport_courier'] . ' : ' . $courier['name']
				  . ' | ' . $GLOBALS['LANG']['transport_type'] . ' : ' . $type['name'] . ' | ' . $GLOBALS['LANG']['transport_region'] . ' : ' . $region['name']
				  . ' | ' . $GLOBALS['LANG']['transport_postcode']
		);

		echo Cms::$twig->render('admin/transport_region_postcode/list.twig', array_merge($data, $params));		
	}
}

$oTransportRegionPostcodeController = new TransportRegionPostcodeController();
$oTransportRegionPostcodeController->init($params);
