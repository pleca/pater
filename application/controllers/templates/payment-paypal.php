...
class Paypal {
	...
	public function listAction() {			
		...
		if($payment['name_url'] != 'paypal') { // wybrano inna metode platnosci
        ...
        if ($this->doPayment($order)) {
			...
			echo Cms::$twig->render('templates/paypal/paypal-express.twig', $data); 			
        }
	}
    ...
    private function doPayment(array $order)
    {
        ...
        $params = array(
            ...
            'payment_type' => OrderLog::PAYMENT_TYPE_PAYPAL,
            'before' => 1
        );
...
//            $oPayPal->setExpressCreditCard(true);
            ...
            $response = $oPayPal->processExpress(RETURN_URL, CANCEL_URL);
            ...
            } else
            {
                ...
                Cms::orderLogSave($_SESSION['order_id'], OrderLog::ACTION_ORDER_PAYMENT, $params);
                $this->order->sendEmailOrderPaymentCustomer($_SESSION['order_id']);
